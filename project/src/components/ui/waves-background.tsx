...
export function Waves({
  ...
  useEffect(() => {
    ...
    function movePoints(time) {
      ...
      lines.forEach((pts) => {
        pts.forEach((p) => {
          ...
          // Only apply cursor interaction on desktop
    ...
    function moved(point, withCursor = true) {
      const x = point.x + point.wave.x + (withCursor && window.innerWidth > 768 ? point.cursor.x : 0)
      const y = point.y + point.wave.y + (withCursor && window.innerWidth > 768 ? point.cursor.y : 0)
    ...
    >
      <div
        className={cn(
          ...
          "md:block hidden" // Hide cursor effect on mobile
        )}
        style={{
          transform:
